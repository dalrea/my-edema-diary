...
(function () {
    ...
    const QUOTES = [
        ...
        '“몸이 힘든 날에도 나는 나를 포기하지 않았습니다.”',
        '“기록은 나를 탓하기 위한 것이 아니라, 나를 이해하기 위한 것입니다.”',
        '“천천히 가도 괜찮습니다. 꾸준함이 가장 큰 치료입니다.”',
    ];
    ...
    /* ---------- AI 1. 마음챙김 문장 생성 ---------- */
    const MIND_MESSAGES = {
        '괜찮아요': '오늘을 무사히 보낸 것만으로도 충분합니다. 작은 기록 하나가 내 몸을 이해하는 시작이 됩니다.',
        '지쳤어요': '지친 날에는 많이 해내지 않아도 괜찮습니다. 쉬는 것도 관리이고, 나를 돌보는 방법입니다.',
        '불안해요': '불안한 마음이 드는 건 자연스러운 일입니다. 오늘의 상태를 기록해두면 내 몸을 조금 더 차분히 살펴볼 수 있습니다.',
        '뿌듯해요': '오늘 해낸 작은 관리가 내일의 가벼움을 만듭니다. 스스로를 충분히 칭찬해 주세요.',
    };
    ...
    function initAiMind() {
        ...
        btn.addEventListener('click', () => {
            if (!selectedMood) {
                // 미선택 시 부드럽게 안내
                title.textContent = '먼저 오늘의 기분을 골라주세요';
                textEl.textContent = '괜찮아요 · 지쳤어요 · 불안해요 · 뿌듯해요 중 하나를 선택해보세요.';
                result.hidden = false;
                return;
            }
            const msg = generateMindCareMessage(selectedMood);
            title.textContent = `AI의 따뜻한 한마디 — “${selectedMood}”`;
    ...
    function summarizeRecord(rawText) {
        ...
        if (exerciseMatch) {
            ...
            items.push({ label: '운동', value: '활동 기록 있음' });
        }
        ...
        if (/압박\s*스타킹|압박스타킹|붕대|압박/.test(text)) {
            ...
            items.push({
                label: '압박 관리',
                value: compTime ? `압박 ${compTime[1]} 착용` : '압박스타킹 착용',
            });
        }

        // 부종 느낌
        if (/무겁|뻐근|땡땡|빵빵|부어|부었|부종|당김/.test(text)) {
            items.push({ label: '부종 느낌', value: '다리가 무거움' });
        } else if (/가벼|편안|괜찮/.test(text)) {
            items.push({ label: '부종 느낌', value: '비교적 가벼움' });
        }

        // 수면 / 수분
        if (/잠|수면|잤|졸/.test(text)) {
            items.push({ label: '수면', value: '수면 관련 기록 있음' });
        }
        if (/물|수분|음용|마셨/.test(text)) {
            items.push({ label: '수분 섭취', value: '수분 섭취 기록 있음' });
        }

        // 비어 있으면 fallback
        if (items.length === 0) {
            items.push({ label: '오늘의 기록', value: '간단한 메모가 저장되었습니다' });
        }

        items.push({
            label: '내일 확인하면 좋은 것',
            value: '수면, 수분 섭취, 압박 착용 시간',
        });
    ...
    function generateCareHint() {
        ...
        return '최근 기록에서는 수면 부족, 압박 착용 시간, 활동량이 부종 느낌과 함께 자주 나타납니다. 내일은 수면 시간과 압박 착용 시간을 함께 기록해보세요.';
    }
    ...
    const INFO_GUIDES = {
        compression: {
            title: '압박 관리',
            text: '압박 스타킹이나 붕대 착용 시간, 불편감, 착용 후 변화를 함께 기록하면 관리 패턴을 이해하는 데 도움이 됩니다.',
        },
        exercise: {
            title: '운동과 스트레칭',
            text: '운동 종류, 시간, 운동 후 다리 느낌을 함께 기록하면 나에게 맞는 활동량을 찾는 데 도움이 됩니다.',
        },
        cellulitis: {
            title: '봉와직염 주의 신호',
            text: '열감, 붉어짐, 통증, 갑작스러운 부종 악화, 발열이 있다면 즉시 의료진과 상담해야 합니다.',
        },
        visit: {
            title: '진료 전 기록 준비',
            text: '최근 부종 변화, 측정 수치, 감염 여부, 압박 착용 시간, 운동 기록을 정리하면 진료 때 상태를 설명하기 쉽습니다.',
        },
    };
    ...
    function summarizeTodayForm() {
        ...
        if (filled.length === 0) {
            return '아직 입력된 항목이 없어요. 한두 가지만 적어도 충분합니다. 천천히 기록해보세요.';
        }
        ...
        return `오늘은 ${friendly}이(가) 기록되었습니다. 내일도 같은 항목을 기록하면 부종 변화의 패턴을 더 잘 확인할 수 있습니다.`;
    }
